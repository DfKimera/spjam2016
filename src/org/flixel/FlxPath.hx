 ...
 */
 class FlxPath {
	 ...
	 */
	public function new(Nodes:Array<ASAny> = null) {
		if (Nodes == null) {
			nodes = [];
		} else {
			nodes = Nodes;
	 ...
	 */
	public function addAt(X:Float, Y:Float, Index:UInt) {
		if (Index > (nodes.length : UInt)) {
			Index = nodes.length;
		}
	 ...
	 */
	public function addPoint(Node:FlxPoint, AsReference:Bool = false) {
		if (AsReference) {
			nodes.push(Node);
		} else {
	 ...
	 * @param	AsReference		Whether to add the point as a reference, or to create a new point with the specified values.
	 */
	public function addPointAt(Node:FlxPoint, Index:UInt, AsReference:Bool = false) {
		if (Index > (nodes.length : UInt)) {
			Index = nodes.length;
		}
		if (AsReference) {
	 ...
	 */
	public function drawDebug(Camera:FlxCamera = null) {
		if (nodes.length <= 0) {
			return;
		}
		if (Camera == null) {
		...
		while (i < l) {
			...
			node = ASCompat.dynamicAs(nodes[i] , FlxPoint);
	...
	@:flash.property public static var manager(get,never):DebugPathDisplay;
static function  get_manager():DebugPathDisplay {
		return Std.downcast(FlxG.getPlugin(DebugPathDisplay) , DebugPathDisplay);
	}
}
