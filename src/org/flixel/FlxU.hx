...
import Globals.flash_utils_getQualifiedClassName as getQualifiedClassName;
 ...
 class FlxU {
	 ...
	 */
	static public function openURL(URL:String) {
	...
	static public function floor(Value:Float):Float {
		var number:Float = Std.int(Value);
	...
	static public function round(Value:Float):Float {
		var number:Float = Std.int(Value + ((Value > 0) ? 0.5 : -0.5));
		return (Value > 0) ? number : ((number != Value) ? (number - 1) : number);
	}
...
;	}
	...
	static public function makeColor(Red:UInt, Green:UInt, Blue:UInt, Alpha:Float = 1.0):UInt {
		return (Std.int((Alpha > 1) ? Std.int(Alpha ): Std.int(Alpha * 255)) & 0xFF) << 24 | (Red & 0xFF) << 16 | (Green & 0xFF) << 8 | (Blue & 0xFF);
	}
	...
	static public function makeColorFromHSB(Hue:Float, Saturation:Float, Brightness:Float, Alpha:Float = 1.0):UInt {
		...
		return (Std.int((Alpha > 1) ? Std.int(Alpha ): Std.int(Alpha * 255)) & 0xFF) << 24 | Std.int(red * 255) << 16 | Std.int(green * 255) << 8 | Std.int(blue * 255);
	}
	 ...
	 */
	static public function getRGBA(Color:UInt, Results:Array<ASAny> = null):Array<ASAny> {
		if (Results == null) {
			Results = [];
		}
	 ...
	 * @return	An <code>Array</code> object containing the Red, Green, Blue and Alpha values of the given color.
	 */
	static public function getHSB(Color:UInt, Results:Array<ASAny> = null):Array<ASAny> {
		if (Results == null) {
			Results = [];
		}
	 ...
	 */
	static public function formatArray(AnyArray:Array<ASAny>):String {
		if ((AnyArray == null) || (AnyArray.length <= 0)) {
			return "";
		}
		var string= Std.string(AnyArray[0]);
		var i:UInt = 0;
	 ...
	 */
	static public function getClassName(Obj:ASAny, Simple:Bool = false):String {
		var string= getQualifiedClassName(Obj);
	 ...
	 */
	static public function compareClassNames(Object1:ASAny, Object2:ASAny):Bool {
	 ...
	 */
	static public function getClass(Name:String):Class<Dynamic> {
