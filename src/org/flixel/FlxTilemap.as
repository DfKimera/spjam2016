...
public class FlxTilemap extends FlxObject {
	...
	static public var ImgAuto:Class;
	[Embed(source="data/autotiles_alt.png")]
	static public var ImgAutoAlt:Class;
	...
	public function getData(Simple:Boolean = false):Array {
		...
		var data:Array = Utils.makeArray(l);
		while (i < l) {
	...
	protected function simplifyPath(Points:Array):void {
		...
		while (i < l) {
			...
			deltaNext = (node.x - Number(Points[i + 1].x)) / (node.y - Number(Points[i + 1].y));
			if ((last.x == Number(Points[i + 1].x)) || (last.y == Number(Points[i + 1].y)) || (deltaPrevious == deltaNext)) {
				Points[i] = null;
			} else {
				last = node;
			}
	...
	protected function computePathDistance(StartIndex:uint, EndIndex:uint):Array {
		...
		var distances:Array = Utils.makeArray(mapSize);
		var i:int = 0;
		while (i < mapSize) {
	 ...
	 */
	public function overlapsWithCallback(obj:FlxObject, Callback:Function = null, FlipCallbackParams:Boolean = false, Position:FlxPoint = null):Boolean {
		var results:Boolean = false;
		...
		var selectionX:int = FlxU.floor((Number(obj.x) - X) / _tileWidth);
		var selectionY:int = FlxU.floor((Number(obj.y) - Y) / _tileHeight);
		var selectionWidth:uint = selectionX + (FlxU.ceil(Number(obj.width) / _tileWidth)) + 1;
		var selectionHeight:uint = selectionY + FlxU.ceil(Number(obj.height) / _tileHeight) + 1;
			...
			while (column < selectionWidth) {
				...
				if (tile.allowCollisions) {
					...
					if (Callback != null) {
						...
						overlapFound = (Number(obj.x) + Number(obj.width) > tile.x) && (Number(obj.x) < tile.x + tile.width) && (Number(obj.y) + Number(obj.height) > tile.y) && (Number(obj.y) < tile.y + tile.height);
					}
					if (overlapFound) {
						if ((tile.callback != null) && ((tile.filter == null) || (obj is tile.filter))) {
							tile.mapIndex = rowStart + column;
							tile.callback(tile, obj);
						}
						results = true;
					}
				} else if ((tile.callback != null) && ((tile.filter == null) || (obj is tile.filter))) {
					tile.mapIndex = rowStart + column;
					tile.callback(tile, obj);
				}
				column++;
	...
	public function getTile(X:uint, Y:uint):uint {
		return uint(_data[Y * widthInTiles + X]);
	}
	...
	protected function updateTile(Index:uint):void {
		...
		var rx:uint = (Number(_data[Index]) - _startingIndex) * _tileWidth;
		var ry:uint = 0;
		if (rx >= _tiles.width) {
