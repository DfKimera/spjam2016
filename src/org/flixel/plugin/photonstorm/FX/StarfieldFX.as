...
public class StarfieldFX extends BaseFX {
	...
	private function update2DStarfield():void {
		for each (var star:* in stars) {
			star.x = Number(star.x) + (starXOffset * Number(star.speed));
			star.y = Number(star.y) + (starYOffset * Number(star.speed));

			canvas.setPixel32(Number(star.x), Number(star.y), depthColours[Number(star.speed) - 1]);
	...
	private function update3DStarfield():void {
		for each (var star:* in stars) {
			star.d = Number(star.d) * 1.1;
			star.x = Number(centerX) + Number((Math.cos(Number(star.r)) * Number(star.d)) * Number(star.speed));
			star.y = Number(centerY) + Number((Math.sin(Number(star.r)) * Number(star.d)) * Number(star.speed));

			star.alpha = Number(star.d) * 2;
