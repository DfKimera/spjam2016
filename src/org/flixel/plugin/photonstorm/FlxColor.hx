 ...
 */
 class FlxColor {
	 ...
	 * @return	Array
	 */
	public static function getHSVColorWheel(alpha:UInt = 255):Array<ASAny> {
		var colors:Array<ASAny> = [];
	 ...
	 */
	public static function getAnalogousHarmony(color:UInt, threshold:Int = 30):ASAny {
		var hsv:ASAny = RGBtoHSV(color);
		...
		var colder= FlxMath.wrapValue(hsv.hue, threshold, 359);

		return {
			color1: color,
			color2: HSVtoRGB(warmer, 1.0, 1.0),
			color3: HSVtoRGB(colder, 1.0, 1.0),
			hue1: hsv.hue,
			hue2: warmer,
			hue3: colder
		}
;	}
	 ...
	 * @return 	Object containing 3 properties: color1 (the original color), color2 (the warmer analogous color) and color3 (the colder analogous color)
	 */
	public static function getSplitComplementHarmony(color:UInt, threshold:Int = 30):ASAny {
		var hsv:ASAny = RGBtoHSV(color);
		...
		var warmer= FlxMath.wrapValue(hsv.hue, opposite - threshold, 359);
		var colder= FlxMath.wrapValue(hsv.hue, opposite + threshold, 359);
		...
		return {
			color1: color,
			color2: HSVtoRGB(warmer, hsv.saturation, hsv.value),
			color3: HSVtoRGB(colder, hsv.saturation, hsv.value),
			hue1: hsv.hue,
			hue2: warmer,
			hue3: colder
		}
;	}
	 ...
	 */
	public static function getTriadicHarmony(color:UInt):ASAny {
		var hsv:ASAny = RGBtoHSV(color);

		var triadic1= FlxMath.wrapValue(hsv.hue, 120, 359);
		var triadic2= FlxMath.wrapValue(triadic1, 120, 359);
...
;	}
	...
	public static function getColorInfo(color:UInt):String {
		var argb:ASAny = getRGB(color);
		var hsl:ASAny = RGBtoHSV(color);

		//	Hex format
		var result= RGBtoHexString(color) + "\n";

		//	RGB format
		result = result + "Alpha: " + argb.alpha + " Red: " + argb.red + " Green: " + argb.green + " Blue: " + argb.blue+ "\n";

		//	HSL info
		result = result + "Hue: " + hsl.hue + " Saturation: " + hsl.saturation + " Lightnes: " + hsl.lightness;
	...
	public static function colorToHexString(color:UInt):String {
		...
		var hexified= digits.charAt(Std.int(msd)) + digits.charAt(Std.int(lsd));
	 ...
	 */
	public static function RGBtoHSV(color:UInt):ASAny {
		var rgb:ASAny = getRGB(color);
	...
	public static function interpolateColor(color1:UInt, color2:UInt, steps:UInt, currentStep:UInt, alpha:UInt = 255):UInt {
		var src1:ASAny = getRGB(color1);
		var src2:ASAny = getRGB(color2);

		var r:UInt = Std.int((((ASCompat.toNumber(src2.red) - ASCompat.toNumber(src1.red)) * currentStep) / steps) + ASCompat.toNumber(src1.red));
		var g:UInt = Std.int((((ASCompat.toNumber(src2.green) - ASCompat.toNumber(src1.green)) * currentStep) / steps) + ASCompat.toNumber(src1.green));
		var b:UInt = Std.int((((ASCompat.toNumber(src2.blue) - ASCompat.toNumber(src1.blue)) * currentStep) / steps) + ASCompat.toNumber(src1.blue));
	...
	public static function interpolateColorWithRGB(color:UInt, r2:UInt, g2:UInt, b2:UInt, steps:UInt, currentStep:UInt):UInt {
		var src:ASAny = getRGB(color);

		var r:UInt = Std.int((((r2 - ASCompat.toNumber(src.red)) * currentStep) / steps) + ASCompat.toNumber(src.red));
		var g:UInt = Std.int((((g2 - ASCompat.toNumber(src.green)) * currentStep) / steps) + ASCompat.toNumber(src.green));
		var b:UInt = Std.int((((b2 - ASCompat.toNumber(src.blue)) * currentStep) / steps) + ASCompat.toNumber(src.blue));
	...
	public static function interpolateRGB(r1:UInt, g1:UInt, b1:UInt, r2:UInt, g2:UInt, b2:UInt, steps:UInt, currentStep:UInt):UInt {
		var r:UInt = Std.int((((r2 - r1) * currentStep) / steps) + r1);
		var g:UInt = Std.int((((g2 - g1) * currentStep) / steps) + g1);
		var b:UInt = Std.int((((b2 - b1) * currentStep) / steps) + b1);
	...
	public static function getRandomColor(min:UInt = 0, max:UInt = 255, alpha:UInt = 255):UInt {
		...
		var red= min + Std.int(Math.random() * (max - min));
		var green= min + Std.int(Math.random() * (max - min));
		var blue= min + Std.int(Math.random() * (max - min));
	 ...
	 */
	public static function getRGB(color:UInt):ASAny {
