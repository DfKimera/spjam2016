 ...
 */
 class FlxMath {
	public static var getrandmax:Int = ASCompat.MAX_INT;
	static var mr:UInt = 0;
	static var cosTable:Array<ASAny> = [];
	static var sinTable:Array<ASAny> = [];

	static var coefficient1:Float = Math.PI / 4;
	static final RADTODEG:Float = 180 / Math.PI;
	static final DEGTORAD:Float = Math.PI / 180;
	...
	public static function mouseInFlxRect(useWorldCoords:Bool, rect:FlxRect):Bool {
		...
		if (useWorldCoords) {
			return pointInFlxRect(Std.int(FlxG.mouse.x), Std.int(FlxG.mouse.y), rect);
		} else {
	...
	public static function atan2(y:Float, x:Float):Float {
		var absY= y;
		var coefficient2= 3 * coefficient1;
		var r:Float;
		var angle:Float;
	 ...
	 * @see getCosTable
	 */
	public static function sinCosGenerator(length:UInt, sinAmplitude:Float = 1.0, cosAmplitude:Float = 1.0, frequency:Float = 1.0):Array<ASAny> {
	 ...
	 * @see sinCosGenerator
	 */
	public static function getSinTable():Array<ASAny> {
		return sinTable;
	}
	 ...
	 * @see sinCosGenerator
	 */
	public static function getCosTable():Array<ASAny> {
		return cosTable;
	}
	...
	public static function rand(min:Float = Math.NaN, max:Float = Math.NaN, excludes:Array<ASAny> = null):Int {
		...
		if (excludes != null) {
			...
			do {
				if (min < max) {
					result = Std.int(min + Math.ffloor(Math.random() * (max + 1 - min))); // Math.floor and adding 1 makes this inclusive
				} else {
					result = Std.int(max + Math.ffloor(Math.random() * (min + 1 - max)));
				}
			}
			...
			//	Reverse check
			if (min < max) {
				return Std.int(min + Math.ffloor(Math.random() * (max + 1 - min)));
			} else {
				return Std.int(max + Math.ffloor(Math.random() * (min + 1 - max)));
			}
		}
	}
	 ...
	 */
	public static function chanceRoll(chance:UInt = 50):Bool {
		if (chance <= 0) {
			return false;
		} else if (chance >= 100) {
			return true;
