 ...
 */
 class DebugPathDisplay extends FlxBasic {
	...
	override public function drawDebug(Camera:FlxCamera = null) {
		...
		var i= _paths.length - 1;
		var path:FlxPath;
		while (i >= 0) {
			path = ASCompat.dynamicAs(_paths[i--] , FlxPath);
	 ...
	 */
	public function add(Path:FlxPath) {
		_paths.push(Path);
	}
	 ...
	 */
	public function remove(Path:FlxPath) {
