 ...
 */
 class TimerManager extends FlxBasic {
	...
	override public function update() {
		var i= _timers.length - 1;
		var timer:FlxTimer;
		while (i >= 0) {
			timer = ASCompat.dynamicAs(_timers[i--] , FlxTimer);
	 ...
	 */
	public function add(Timer:FlxTimer) {
		_timers.push(Timer);
	}
	 ...
	 */
	public function remove(Timer:FlxTimer) {
