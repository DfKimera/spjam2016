 ...
 */
 class FlxTilemap extends FlxObject {
	...
	static public var ImgAuto:Class<Dynamic>;
	@:meta(Embed(source="data/autotiles_alt.png"))
	static public var ImgAutoAlt:Class<Dynamic>;

	/**
	 * No auto-tiling.
	 */
	static public inline final OFF:UInt = 0;
	/**
	 * Good for levels with thin walls that don'tile need interior corner art.
	 */
	static public inline final AUTO:UInt = 1;
	/**
	 * Better for levels with thick walls that look better with interior corner art.
	 */
	static public inline final ALT:UInt = 2;
	...
	override public function destroy() {
		...
		while (i < l) {
			ASCompat.dynamicAs(_tileObjects[i++] , FlxTile).destroy();
		}
		_tileObjects = null;
		i = 0;
		l = _buffers.length;
		while (i < l) {
			ASCompat.dynamicAs(_buffers[i++] , FlxTilemapBuffer).destroy();
		}
		_buffers = null;
		_data = null;
		_rects = null;
	...
	public function loadMap(MapData:String, TileGraphic:String, TileWidth:UInt = 0, TileHeight:UInt = 0, AutoTile:UInt = OFF, StartingIndex:UInt = 0, DrawIndex:UInt = 1, CollideIndex:UInt = 1):FlxTilemap {
		...
		var columns:Array<ASAny>;
		var rows:Array<ASAny> = (cast MapData.split("\n"));
		...
		while (row < heightInTiles) {
			...
			while (column < widthInTiles) {
				_data.push(ASCompat.toInt(columns[column++]));
			}
		}
		...
		i = 0;
		var l:UInt = Std.int((_tiles.width / _tileWidth) * (_tiles.height / _tileHeight));
		if (auto > OFF) {
			l++;
		}
		_tileObjects = Utils.makeArray(l);
		var ac:UInt;
		while (i < l) {
		...
		_rects = Utils.makeArray(totalTiles);
		i = 0;
		while (i < totalTiles) {
			updateTile(i++);
		}
	 ...
	 */
	function makeDebugTile(Color:UInt):BitmapData {
	 ...
	 */
	function drawTilemap(Buffer:FlxTilemapBuffer, Camera:FlxCamera) {
		Buffer.fill();
		...
		var screenXInTiles= Std.int((_point.x + ((_point.x > 0) ? 0.0000001 : -0.0000001)) / _tileWidth);
		var screenYInTiles= Std.int((_point.y + ((_point.y > 0) ? 0.0000001 : -0.0000001)) / _tileHeight);
		...
		if ((screenXInTiles : UInt) > widthInTiles - screenColumns) {
			screenXInTiles = widthInTiles - screenColumns;
		}
		if (screenYInTiles < 0) {
			screenYInTiles = 0;
		}
		if ((screenYInTiles : UInt) > heightInTiles - screenRows) {
		...
		while (row < screenRows) {
			...
			while (column < screenColumns) {
				_flashRect = ASCompat.dynamicAs(_rects[columnIndex] , Rectangle);
				if (_flashRect != null) {
	...
	override public function draw() {
		...
		while (i < l) {
			...
			buffer = ASCompat.dynamicAs(_buffers[i++] , FlxTilemapBuffer);
			if (!buffer.dirty) {
	 ...
	 */
	public function getData(Simple:Bool = false):Array<ASAny> {
		if (!Simple) {
			return _data;
		}
		...
		var data= Utils.makeArray(l);
		while (i < l) {
			data[i] = (ASCompat.dynamicAs(_tileObjects[_data[i]] , FlxTile).allowCollisions > 0) ? 1 : 0;
			i++;
		}
		return data;
	}
	 ...
	 */
	public function setDirty(Dirty:Bool = true) {
		var i:UInt = 0;
		var l:UInt = _buffers.length;
		while (i < l) {
			ASCompat.dynamicAs(_buffers[i++] , FlxTilemapBuffer).dirty = Dirty;
		}
	}
	...
	public function findPath(Start:FlxPoint, End:FlxPoint, Simplify:Bool = true, RaySimplify:Bool = false):FlxPath {
		...
		var startIndex= Std.int((Start.y - y) / _tileHeight) * widthInTiles + Std.int((Start.x - x) / _tileWidth);
		var endIndex= Std.int((End.y - y) / _tileHeight) * widthInTiles + Std.int((End.x - x) / _tileWidth);

		//check that the start and end are clear.
		if ((ASCompat.dynamicAs(_tileObjects[_data[startIndex]] , FlxTile).allowCollisions > 0) ||
				(ASCompat.dynamicAs(_tileObjects[_data[endIndex]] , FlxTile).allowCollisions > 0)) {
			return null;
		}

		//figure out how far each of the tiles is from the starting tile
		var distances= computePathDistance(startIndex, endIndex);
		if (distances == null) {
			return null;
		}

		//then count backward to find the shortest path.
		var points:Array<ASAny> = [];
		walkPath(distances, endIndex, points);

		//reset the start and end points to be exact
		var node:FlxPoint;
		node = ASCompat.dynamicAs(points[points.length - 1] , FlxPoint);
		node.x = Start.x;
		node.y = Start.y;
		node = ASCompat.dynamicAs(points[0] , FlxPoint);
		node.x = End.x;
		node.y = End.y;
		...
		while (i >= 0) {
			node = ASCompat.dynamicAs(points[i--] , FlxPoint);
			if (node != null) {
	 ...
	 */
	function simplifyPath(Points:Array<ASAny>) {
		...
		while (i < l) {
			...
			deltaNext = (node.x - ASCompat.toNumber(Points[i + 1].x)) / (node.y - ASCompat.toNumber(Points[i + 1].y));
			if ((last.x == ASCompat.toNumber(Points[i + 1].x)) || (last.y == ASCompat.toNumber(Points[i + 1].y)) || (deltaPrevious == deltaNext)) {
				Points[i] = null;
			} else {
				last = node;
			}
	 ...
	 * @param	Points		An array of <code>FlxPoint</code> nodes.
	 */
	function raySimplifyPath(Points:Array<ASAny>) {
	 ...
	 */
	function computePathDistance(StartIndex:UInt, EndIndex:UInt):Array<ASAny> {
		//Create a distance-based representation of the tilemap.
		//All walls are flagged as -2, all open areas as -1.
		var mapSize= widthInTiles * heightInTiles;
		var distances= Utils.makeArray(mapSize);
		var i= 0;
		while ((i : UInt) < mapSize) {
			if (ASCompat.dynamicAs(_tileObjects[_data[i]] , FlxTile).allowCollisions != 0) {
				distances[i] = -2;
			} else {
				distances[i] = -1;
			}
		...
		var distance:UInt = 1;
		var neighbors:Array<ASAny> = [StartIndex];
		var current:Array<ASAny>;
	 ...
	 */
	function walkPath(Data:Array<ASAny>, Start:UInt, Points:Array<ASAny>) {
		Points.push(new FlxPoint(x + Start % widthInTiles * _tileWidth + _tileWidth * 0.5, y + Std.int(Start / widthInTiles) * _tileHeight + _tileHeight * 0.5));
		if (Data[Start] == 0) {
			return;
		}

		//basic map bounds
		var left= Start % widthInTiles > 0;
		var right= Start % widthInTiles < widthInTiles - 1;
		var up= Start / widthInTiles > 0;
		var down= Start / widthInTiles < heightInTiles - 1;
	...
	override public function overlaps(ObjectOrGroup:FlxBasic, InScreenSpace:Bool = false, Camera:FlxCamera = null):Bool {
		if (Std.is(ObjectOrGroup , FlxGroup)) {
			...
			var basic:FlxBasic;
			var i:UInt = 0;
			var members= Std.downcast(ObjectOrGroup , FlxGroup).members;
			while (i < (members.length : UInt)) {
				basic = ASCompat.dynamicAs(members[i++] , FlxBasic);
				if (Std.is(basic , FlxObject)) {
					if (overlapsWithCallback(Std.downcast(basic , FlxObject))) {
						results = true;
					}
				} else {
			...
			return overlapsWithCallback(Std.downcast(ObjectOrGroup , FlxObject));
		}
		return false;
	}
		...
		if (Std.is(ObjectOrGroup , FlxGroup)) {
			...
			while (i < (members.length : UInt)) {
				...
				if (Std.is(basic , FlxObject)) {
					_point.x = X;
					_point.y = Y;
					if (overlapsWithCallback(Std.downcast(basic , FlxObject), null, false, _point)) {
						results = true;
					}
				} else {
			...
			_point.x = X;
			_point.y = Y;
			return overlapsWithCallback(Std.downcast(ObjectOrGroup , FlxObject), null, false, _point);
		}
		return false;
	}
	 ...
	 *
	 * @param	obj				The <code>FlxObject</code> you are checking for overlaps against.
	 ...
	 */
	public function overlapsWithCallback(obj:FlxObject, Callback:ASFunction = null, FlipCallbackParams:Bool = false, Position:FlxPoint = null):Bool {
		var results= false;
		...
		var selectionX= Std.int(FlxU.floor((obj.x - X) / _tileWidth));
		var selectionY= Std.int(FlxU.floor((obj.y - Y) / _tileHeight));
		var selectionWidth:UInt = Std.int(selectionX + FlxU.ceil(obj.width / _tileWidth) + 1);
		var selectionHeight:UInt = Std.int(selectionY + FlxU.ceil(obj.height / _tileHeight) + 1);
		...
		var rowStart= selectionY * widthInTiles;
		...
		while (row < selectionHeight) {
			...
			while (column < selectionWidth) {
				overlapFound = false;
				tile = ASCompat.dynamicAs(_tileObjects[_data[rowStart + column]] , FlxTile);
				if (tile.allowCollisions != 0) {
					...
					if (Callback != null) {
						if (FlipCallbackParams) {
							overlapFound = Callback(obj, tile);
						} else {
							overlapFound = Callback(tile, obj);
						}
					} else {
						overlapFound = (obj.x + obj.width > tile.x) && (obj.x < tile.x + tile.width) && (obj.y + obj.height > tile.y) && (obj.y < tile.y + tile.height);
					}
					if (overlapFound) {
						if ((tile.callback != null) && ((tile.filter == null) || Std.is(obj , tile.filter))) {
							tile.mapIndex = rowStart + column;
							tile.callback(tile, obj);
						}
						results = true;
					}
				} else if ((tile.callback != null) && ((tile.filter == null) || Std.is(obj , tile.filter))) {
					tile.mapIndex = rowStart + column;
					tile.callback(tile, obj);
				}
				column++;
	...
	override public function overlapsPoint(Point:FlxPoint, InScreenSpace:Bool = false, Camera:FlxCamera = null):Bool {
		if (!InScreenSpace) {
			return ASCompat.dynamicAs(_tileObjects[_data[Std.int(Std.int((Point.y - y) / _tileHeight) * widthInTiles + (Point.x - x) / _tileWidth)]] , FlxTile).allowCollisions > 0;
		}
		...
		return ASCompat.dynamicAs(_tileObjects[_data[Std.int(Std.int((Point.y - _point.y) / _tileHeight) * widthInTiles + (Point.x - _point.x) / _tileWidth)]] , FlxTile).allowCollisions > 0;
	}
	...
	public function getTile(X:UInt, Y:UInt):UInt {
		return ASCompat.toInt(_data[Y * widthInTiles + X]);
	}
	...
	public function getTileByIndex(Index:UInt):UInt {
		return ASCompat.toInt(_data[Index]);
	}
	 ...
	 */
	public function getTileInstances(Index:UInt):Array<ASAny> {
		var array:Array<ASAny> = null;
		var i:UInt = 0;
		var l= widthInTiles * heightInTiles;
		while (i < l) {
			if (_data[i] == Index) {
				if (array == null) {
					array = [];
	 ...
	 */
	public function getTileCoords(Index:UInt, Midpoint:Bool = true):Array<ASAny> {
		var array:Array<ASAny> = null;
		...
		while (i < l) {
			if (_data[i] == Index) {
				point = new FlxPoint(x + i % widthInTiles * _tileWidth, y + Std.int(i / widthInTiles) * _tileHeight);
				if (Midpoint) {
	...
	public function setTileByIndex(Index:UInt, Tile:UInt, UpdateGraphics:Bool = true):Bool {
		...
		var i:UInt;
		var row= Std.int(Index / widthInTiles) - 1;
		var rowLength= row + 3;
		...
		while (row < rowLength) {
			...
			while (column < columnHeight) {
				if ((row >= 0) && ((row : UInt) < heightInTiles) && (column >= 0) && ((column : UInt) < widthInTiles)) {
	 ...
	 */
	public function setTileProperties(Tile:UInt, AllowCollisions:UInt = 0x1111, Callback:ASFunction = null, CallbackFilter:Class<Dynamic> = null, Range:UInt = 1) {
		if (Range <= 0) {
			Range = 1;
		}
		var tile:FlxTile;
		...
		var l= Tile + Range;
		while (i < l) {
			tile = ASCompat.dynamicAs(_tileObjects[i++] , FlxTile);
	 ...
	 */
	public function follow(Camera:FlxCamera = null, Border:Int = 0, UpdateWorld:Bool = true) {
		if (Camera == null) {
			Camera = FlxG.camera;
		}
	...
	public function ray(Start:FlxPoint, End:FlxPoint, Result:FlxPoint = null, Resolution:Float = 1):Bool {
		...
		var curX= Start.x - stepX - x;
		var curY= Start.y - stepY - y;
		var tileX:UInt;
		var tileY:UInt;
		var i:UInt = 0;
		while (i < steps) {
			...
			tileX = Std.int(curX / _tileWidth);
			tileY = Std.int(curY / _tileHeight);
			if (ASCompat.dynamicAs(_tileObjects[_data[tileY * widthInTiles + tileX]] , FlxTile).allowCollisions != 0) {
	 ...
	 */
	static public function arrayToCSV(Data:Array<ASAny>, Width:Int, Invert:Bool = false):String {
		var row:UInt = 0;
		var column:UInt;
		var csv:String = null;
		var Height= Std.int(Data.length / Width);
		var index:Int;
		while (row < (Height : UInt)) {
			column = 0;
			while (column < (Width : UInt)) {
	 ...
	 * @return	A comma-separated string containing the level data in a <code>FlxTilemap</code>-friendly format.
	 */
	static public function imageToCSV(ImageFile:Class<Dynamic>, Invert:Bool = false, Scale:UInt = 1):String {
	 ...
	 */
	function updateTile(Index:UInt) {
		var tile= ASCompat.dynamicAs(_tileObjects[_data[Index]] , FlxTile);
		if ((tile == null) || !tile.visible) {
			_rects[Index] = null;
			return;
		}
		var rx:UInt = Std.int((ASCompat.toNumber(_data[Index]) - _startingIndex) * _tileWidth);
		var ry:UInt = 0;
		if (rx >= (_tiles.width : UInt)) {
