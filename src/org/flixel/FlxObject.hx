 ...
 */
 class FlxObject extends FlxBasic {
	 ...
	 */
	public var elasticity:Float = Math.NaN;
	/**
	 ...
	 * Internal helper used for retro-style flickering.
	 */
	var _flickerTimer:Float = Math.NaN;
	/**
	 * Handy for storing health percentage or armor points or whatever.
	 */
	public var health:Float = Math.NaN;
	/**
	 ...
	 */
	public var touching:UInt = 0;
	/**
	 * Bit field of flags (use with UP, DOWN, LEFT, RIGHT, etc) indicating surface contacts from the previous game loop step.
	 * Use bitwise operators to check the values stored here, or use touching(), justStartedTouching(), etc.
	 * You can even use them broadly as boolean values if you're feeling saucy!
	 */
	public var wasTouching:UInt = 0;
	/**
	 ...
	 */
	public var allowCollisions:UInt = 0;
	 ...
	 */
	override public function drawDebug(Camera:FlxCamera = null) {
		if (Camera == null) {
			Camera = FlxG.camera;
		}

		//get bounding box coordinates
		var boundingBoxX= x - Std.int(Camera.scroll.x * scrollFactor.x); //copied from getScreenXY()
		var boundingBoxY= y - Std.int(Camera.scroll.y * scrollFactor.y);
		...
		var boundingBoxWidth= Std.int((width != Std.int(width)) ? Std.int(width ): Std.int(width - 1));
		var boundingBoxHeight= Std.int((height != Std.int(height)) ? Std.int(height ): Std.int(height - 1));
	 ...
	 */
	public function followPath(Path:FlxPath, Speed:Float = 100, Mode:UInt = PATH_FORWARD, AutoRotate:Bool = false) {
	 ...
	 */
	public function stopFollowingPath(DestroyPath:Bool = false) {
		pathSpeed = 0;
	 ...
	 */
	function updatePathMotion() {
		//first check if we need to be pointing at the next node yet
		_point.x = x + width * 0.5;
		_point.y = y + height * 0.5;
		var node:FlxPoint = path.nodes[_pathNodeIndex];
		var deltaX= node.x - _point.x;
		var deltaY= node.y - _point.y;

		var horizontalOnly= (_pathMode & PATH_HORIZONTAL_ONLY) > 0;
		var verticalOnly= (_pathMode & PATH_VERTICAL_ONLY) > 0;
	...
	public function overlaps(ObjectOrGroup:FlxBasic, InScreenSpace:Bool = false, Camera:FlxCamera = null):Bool {
		...
		if (Std.is(ObjectOrGroup , FlxTilemap)) {
			...
			return Std.downcast(ObjectOrGroup , FlxTilemap).overlaps(this, InScreenSpace, Camera);
		}

		var object= Std.downcast(ObjectOrGroup , FlxObject);
		if (!InScreenSpace) {
	...
	public function overlapsAt(X:Float, Y:Float, ObjectOrGroup:FlxBasic, InScreenSpace:Bool = false, Camera:FlxCamera = null):Bool {
		...
		if (Std.is(ObjectOrGroup , FlxTilemap)) {
			...
			var tilemap= Std.downcast(ObjectOrGroup , FlxTilemap);
	 ...
	 */
	public function flicker(Duration:Float = 1) {
	 ...
	 */
	@:flash.property public var flickering(get,never):Bool;
function  get_flickering():Bool {
	 ...
	 * @private
	 */
	@:flash.property public var solid(get,set):Bool;
function  get_solid():Bool {
		return ((allowCollisions & ANY) : UInt) > NONE;
	}
...
return Solid;
	...
	public function isTouching(Direction:UInt):Bool {
		return ((touching & Direction) : UInt) > NONE;
	}
	...
	public function justTouched(Direction:UInt):Bool {
		return (((touching & Direction) : UInt) > NONE) && (((wasTouching & Direction) : UInt) <= NONE);
	}
	...
	static public function separate(Object1:FlxObject, Object2:FlxObject):Bool {
		var separatedX= separateX(Object1, Object2);
		var separatedY= separateY(Object1, Object2);
	...
	static public function separateX(Object1:FlxObject, Object2:FlxObject):Bool {
		...
		if (Std.is(Object1 , FlxTilemap)) {
			return Std.downcast(Object1 , FlxTilemap).overlapsWithCallback(Object2, separateX);
		}
		if (Std.is(Object2 , FlxTilemap)) {
			return Std.downcast(Object2 , FlxTilemap).overlapsWithCallback(Object1, separateX, true);
		}
		...
		if (obj1delta != obj2delta) {
			...
			var obj1rect= new FlxRect(Object1.x - ((obj1delta > 0) ? obj1delta : 0), Object1.last.y, Object1.width + ((obj1delta > 0) ? obj1delta : -obj1delta), Object1.height);
			var obj2rect= new FlxRect(Object2.x - ((obj2delta > 0) ? obj2delta : 0), Object2.last.y, Object2.width + ((obj2delta > 0) ? obj2delta : -obj2delta), Object2.height);
	...
	static public function separateY(Object1:FlxObject, Object2:FlxObject):Bool {
		...
		//If one of the objects is a tilemap, just pass it off.
		if (Std.is(Object1 , FlxTilemap)) {
			return Std.downcast(Object1 , FlxTilemap).overlapsWithCallback(Object2, separateY);
		}
		if (Std.is(Object2 , FlxTilemap)) {
			return Std.downcast(Object2 , FlxTilemap).overlapsWithCallback(Object1, separateY, true);
		}
		...
		if (obj1delta != obj2delta) {
			...
			var obj1deltaAbs= (obj1delta > 0) ? obj1delta : -obj1delta;
			var obj2deltaAbs= (obj2delta > 0) ? obj2delta : -obj2delta;
			var obj1rect= new FlxRect(Object1.x, Object1.y - ((obj1delta > 0) ? obj1delta : 0), Object1.width, Object1.height + obj1deltaAbs);
			var obj2rect= new FlxRect(Object2.x, Object2.y - ((obj2delta > 0) ? obj2delta : 0), Object2.width, Object2.height + obj2deltaAbs);
			if ((obj1rect.x + obj1rect.width > obj2rect.x) && (obj1rect.x < obj2rect.x + obj2rect.width) && (obj1rect.y + obj1rect.height > obj2rect.y) && (obj1rect.y < obj2rect.y + obj2rect.height)) {
				var maxOverlap= obj1deltaAbs + obj2deltaAbs + OVERLAP_BIAS;
