...
public class FrameRecord {
	...
	public function load(Data:String):FrameRecord {
		...
		frame = int(String(array[0]));

		//split up keyboard and mouse data
		array = (String(array[1])).split("m");
		...
		if (keyData.length > 0) {
			...
			l = array.length;
			while (i < l) {
				keyPair = (String(array[i++])).split(":");
				if (keyPair.length == 2) {
					if (keys == null) {
						keys = [];
					}
					keys.push({code: int(String(keyPair[0])), value: int(String(keyPair[1]))});
				}
			}
		}
		...
		if (mouseData.length > 0) {
			...
			if (array.length >= 4) {
				mouse = new MouseRecord(int(String(array[0])), int(String(array[1])), int(String(array[2])), int(String(array[3])));
			}
		}
