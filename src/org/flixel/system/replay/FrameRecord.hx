 ...
 */
 class FrameRecord {
	 ...
	 *
	 */
	public function create(Frame:Float, Keys:Array<ASAny> = null, Mouse:MouseRecord = null):FrameRecord {
		frame = Std.int(Frame);
		keys = Keys;
		mouse = Mouse;
		return this;
	...
	public function load(Data:String):FrameRecord {
		...
		//get frame number
		var array:Array<ASAny> = (cast Data.split("k"));
		frame = ASCompat.toInt(ASCompat.toString(array[0]));

		//split up keyboard and mouse data
		array = (cast ASCompat.toString(array[1]).split("m"));
		...
		if (keyData.length > 0) {
			...
			array = (cast keyData.split(","));

			//go through each data pair and enter it into this frame's key state
			var keyPair:Array<ASAny>;
			i = 0;
			l = array.length;
			while (i < l) {
				keyPair = (cast ASCompat.toString(array[i++]).split(":"));
				if (keyPair.length == 2) {
					if (keys == null) {
						keys = [];
					}
					keys.push({code: ASCompat.toInt(ASCompat.toString(keyPair[0])), value: ASCompat.toInt(ASCompat.toString(keyPair[1]))});
				}
			}
		}
		...
		if (mouseData.length > 0) {
			array = (cast mouseData.split(","));
			if (array.length >= 4) {
				mouse = new MouseRecord(ASCompat.toInt(ASCompat.toString(array[0])), ASCompat.toInt(ASCompat.toString(array[1])), ASCompat.toInt(ASCompat.toString(array[2])), ASCompat.toInt(ASCompat.toString(array[3])));
			}
		}
