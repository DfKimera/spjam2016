 ...
 */
 class VCR extends Sprite {
	...
	var _open:Bitmap;
	var _recordOff:Bitmap;
	var _recordOn:Bitmap;
	var _stop:Bitmap;
	var _flixel:Bitmap;
	var _restart:Bitmap;
	var _pause:Bitmap;
	...
	var _overOpen:Bool = false;
	var _overRecord:Bool = false;
	var _overRestart:Bool = false;
	var _overPause:Bool = false;
	var _overStep:Bool = false;

	var _pressingOpen:Bool = false;
	var _pressingRecord:Bool = false;
	var _pressingRestart:Bool = false;
	var _pressingPause:Bool = false;
	var _pressingStep:Bool = false;

	var _file:FileReference;

	var _runtimeDisplay:TextField;
	var _runtime:UInt = 0;
	 ...
	 */
	public function updateRuntime(Time:UInt) {
		_runtime += Time;
	 ...
	 * @param	E	Flash event.
	 */
	function onOpenSelect(E:Event = null) {
	 ...
	 * @param	E	Flash Event.
	 */
	function onOpenComplete(E:Event = null) {
	 ...
	 * @param	E	Flash Event.
	 */
	function onOpenCancel(E:Event = null) {
		_file.removeEventListener(Event.SELECT, onOpenSelect);
		_file.removeEventListener(Event.CANCEL, onOpenCancel);
		_file = null;
	}
	 ...
	 * @param	E	Flash Event.
	 */
	function onOpenError(E:Event = null) {
		_file.removeEventListener(Event.COMPLETE, onOpenComplete);
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onOpenError);
		_file = null;
	 ...
	 */
	public function onRecord(StandardMode:Bool = false) {
		if (_play.visible) {
			onPlay();
		}
	 ...
	 * @param	E	Flash Event.
	 */
	function onSaveComplete(E:Event = null) {
	 ...
	 * @param	E	Flash Event.
	 */
	function onSaveCancel(E:Event = null) {
		_file.removeEventListener(Event.COMPLETE, onSaveComplete);
		_file.removeEventListener(Event.CANCEL, onSaveCancel);
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
	 ...
	 * @param	E	Flash Event.
	 */
	function onSaveError(E:Event = null) {
		_file.removeEventListener(Event.COMPLETE, onSaveComplete);
		_file.removeEventListener(Event.CANCEL, onSaveCancel);
		_file.removeEventListener(IOErrorEvent.IO_ERROR, onSaveError);
		_file = null;
	 ...
	 */
	public function onRestart(StandardMode:Bool = false) {
	 ...
	 * @param	E	Flash mouse event.
	 */
	function onMouseDown(E:MouseEvent = null) {
		unpress();
		if (_overOpen) {
			_pressingOpen = true;
		}
