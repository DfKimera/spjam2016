 ...
 */
 class FlxButton extends FlxSprite {
	 ...
	 */
	public function new(X:Float = 0, Y:Float = 0, Label:String = null, OnClick:ASFunction = null) {
		super(X, Y);
		if (Label != null) {
	...
	override public function update() {
		...
		switch (frame) {
			case (_ == HIGHLIGHT => true):	//Extra behavior to accomodate checkbox logic.
				label.alpha = 1.0;
	...
	function updateButton() {
		...
		if (FlxG.mouse.visible) {
			...
			var offAll= true;
			while (i < l) {
				camera = ASCompat.dynamicAs(cameras[i++] , FlxCamera);
	 ...
	 */
	public function setSounds(SoundOver:Class<Dynamic> = null, SoundOverVolume:Float = 1.0, SoundOut:Class<Dynamic> = null, SoundOutVolume:Float = 1.0, SoundDown:Class<Dynamic> = null, SoundDownVolume:Float = 1.0, SoundUp:Class<Dynamic> = null, SoundUpVolume:Float = 1.0) {
		if (SoundOver != null) {
	 ...
	 * @private
	 */
	@:flash.property public var on(get,set):Bool;
