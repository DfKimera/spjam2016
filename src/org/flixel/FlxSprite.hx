 ...
 */
 class FlxSprite extends FlxObject {
	 ...
	 */
	var _frameTimer:Float = Math.NaN;
	/**
	 ...
	 */
	var _bakedRotation:Float = Math.NaN;
	/**
	...
	public function loadRotatedGraphic(Graphic:String, Rotations:UInt = 16, Frame:Int = -1, AntiAliasing:Bool = false, AutoBuffer:Bool = false):FlxSprite {
		...
		if ((brush.height : UInt) > max) {
			max = brush.height;
		}
		if (AutoBuffer) {
			max = Std.int(max * 1.5);
		}
		var columns:UInt = Std.int(FlxU.ceil(Rotations / rows));
		width = max * columns;
		height = max * rows;
		var key= ASCompat.toString(Graphic) + ":" + Frame + ":" + width + "x" + height;
		...
		if (!skipGen) {
			...
			var halfBrushWidth:UInt = Std.int(brush.width * 0.5);
			var halfBrushHeight:UInt = Std.int(brush.height * 0.5);
			var midpointX:UInt = Std.int(max * 0.5);
			var midpointY:UInt = Std.int(max * 0.5);
			while (row < rows) {
				column = 0;
		...
		frameWidth = frameHeight = Std.int(width = height = max);
		resetHelpers();
		if (AutoBuffer) {
			width = brush.width;
			height = brush.height;
	...
	override public function draw() {
		...
		while (i < l) {
			...
			if (((angle == 0) || (_bakedRotation > 0)) && (scale.x == 1) && (scale.y == 1) && (blend == null)) {	//Simple render
	 ...
	 */
	public function stamp(Brush:FlxSprite, X:Int = 0, Y:Int = 0) {
		Brush.drawFrame();
	 ...
	 */
	public function drawLine(StartX:Float, StartY:Float, EndX:Float, EndY:Float, Color:UInt, Thickness:UInt = 1) {
		//Draw line
		var gfx= FlxG.flashGfx;
		gfx.clear();
		gfx.moveTo(StartX, StartY);
		var alphaComponent= ((Color >> 24) & 0xFF) / 255;
		if (alphaComponent <= 0) {
			alphaComponent = 1;
		}
	 ...
	 */
	public function fill(Color:UInt) {
	...
	function updateAnimation() {
		if (_bakedRotation > 0) {
			var oldIndex= _curIndex;
			var angleHelper= Std.int(angle % 360);
			if (angleHelper < 0) {
				angleHelper += 360;
			}
			...
			while (_frameTimer > _curAnim.delay) {
				...
				if (_curFrame == (_curAnim.frames.length - 1 : UInt)) {
					if (_curAnim.looped) {
						_curFrame = 0;
					}
					finished = true;
	 ...
	 */
	public function drawFrame(Force:Bool = false) {
		if (Force || dirty) {
			calcFrame();
		}
	}
	 ...
	 */
	public function addAnimation(Name:String, Frames:Array<ASAny>, FrameRate:Float = 0, Looped:Bool = true) {
	 ...
	 */
	public function addAnimationCallback(AnimationCallback:ASFunction) {
	 ...
	 */
	public function setOriginToCorner() {
		origin.x = origin.y = 0;
	}
	 ...
	 */
	public function centerOffsets(AdjustPosition:Bool = false) {
	...
	public function replaceColor(Color:UInt, NewColor:UInt, FetchPositions:Bool = false):Array<ASAny> {
		var positions:Array<ASAny> = null;
		if (FetchPositions) {
			positions = [];
		}
	 ...
	 * @private
	 */
	@:flash.property public var pixels(get,set):BitmapData;
...
return Pixels;
	 ...
	 * @private
	 */
	@:flash.property public var facing(get,set):UInt;
function  get_facing():UInt {
		return _facing;
	}
function  set_facing(Direction:UInt):UInt{
...
return Color;
	 ...
	 * @private
	 */
	@:flash.property public var frame(get,set):UInt;
function  get_frame():UInt {
		return _curIndex;
	}
function  set_frame(Frame:UInt):UInt{
		_curAnim = null;
		_curIndex = Frame;
		dirty = true;
return Frame;
	...
	override public function onScreen(Camera:FlxCamera = null):Bool {
		...
		var absScaleX= (scale.x > 0) ? scale.x : -scale.x;
		var absScaleY= (scale.y > 0) ? scale.y : -scale.y;
	 ...
	 */
	function calcFrame() {
		var indexX= _curIndex * frameWidth;
		var indexY:UInt = 0;
