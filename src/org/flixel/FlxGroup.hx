 ...
 */
 class FlxGroup extends FlxBasic {
	 ...
	 */
	static public inline final ASCENDING= -1;
	/**
	 * Use with <code>sort()</code> to sort in descending order.
	 */
	static public inline final DESCENDING= 1;

	/**
	 * Array of all the <code>FlxBasic</code>s that exist in this group.
	 */
	public var members:Array<ASAny>;
	/**
	 ...
	 * @private
	 */
	@:flash.property public var maxSize(get,set):UInt;
function  get_maxSize():UInt {
		return _maxSize;
	}
function  set_maxSize(Size:UInt):UInt{
		_maxSize = Size;
		if (_marker >= _maxSize) {
			_marker = 0;
		}
		if ((_maxSize == 0) || (members == null) || (_maxSize >= (members.length : UInt))) {
			return Size;
		}
		...
		length = ASCompat.arraySetLength(members, _maxSize);
return Size;
	}
	...
	public function add(obj:FlxBasic):FlxBasic {
		...
		if (_maxSize > 0) {
			if ((members.length : UInt) >= _maxSize) {
				return obj;
			} else if ((members.length * 2 : UInt) <= _maxSize) {
				members.resize(members.length * 2);
			} else {
				members.resize(_maxSize);
			}
		} else {
			members.resize(members.length * 2);
		}
	 ...
	 */
	public function recycle(ObjectClass:Class<Dynamic> = null):FlxBasic {
		var basic:FlxBasic;
		if (_maxSize > 0) {
			if (length < _maxSize) {
				if (ObjectClass == null) {
				...
				return add(ASCompat.dynamicAs(Type.createInstance(ObjectClass, []) , FlxBasic));
			} else {
	 ...
	 */
	public function sort(Index:String = "y", Order:Int = ASCENDING) {
		_sortIndex = Index;
		_sortOrder = Order;
	 ...
	 */
	public function setAll(VariableName:String, Value:ASAny, Recurse:Bool = true) {
		var basic:FlxBasic;
		var i:UInt = 0;
		while (i < length) {
			basic = ASCompat.dynamicAs(members[i++] , FlxBasic);
			if (basic != null) {
				if (Recurse && Std.is(basic , FlxGroup)) {
					Std.downcast(basic , FlxGroup).setAll(VariableName, Value, Recurse);
				} else {
					(basic : ASAny)[VariableName] = Value;
				}
			}
		}
	 ...
	 */
	public function callAll(FunctionName:String, Recurse:Bool = true) {
		var basic:FlxBasic;
		var i:UInt = 0;
		while (i < length) {
			basic = ASCompat.dynamicAs(members[i++] , FlxBasic);
			if (basic != null) {
				if (Recurse && Std.is(basic , FlxGroup)) {
					Std.downcast(basic , FlxGroup).callAll(FunctionName, Recurse);
				} else {
					(basic : ASAny)[FunctionName]();
				}
			}
		}
	}
	 ...
	 */
	public function getFirstAvailable(ObjectClass:Class<Dynamic> = null):FlxBasic {
		var basic:FlxBasic;
		var i:UInt = 0;
		while (i < length) {
			basic = ASCompat.dynamicAs(members[i++] , FlxBasic);
	...
	public function getRandom(StartIndex:UInt = 0, Length:UInt = 0):FlxBasic {
		if (Length == 0) {
			Length = Std.int(length);
		}
		return ASCompat.dynamicAs(FlxG.getRandom(members, StartIndex, Length) , FlxBasic);
	}
	...
	public function clear() {
		length = ASCompat.arraySetLength(members, 0);
	}
	 ...
	 */
	function sortHandler(Obj1:FlxBasic, Obj2:FlxBasic):Int {
		if ((Obj1 : ASAny)[_sortIndex] < (Obj2 : ASAny)[_sortIndex]) {
			return _sortOrder;
		} else if ((Obj1 : ASAny)[_sortIndex] > (Obj2 : ASAny)[_sortIndex]) {
			return -_sortOrder;
		}
		return 0;
	}
