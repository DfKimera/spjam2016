...
public class EventChain {
	...
	public function nextEvent():void {
		...
		if (event.newBackground != null) {
			displayBackground(event.newBackground);
		}
		if (event.newBGM != null) {
			Game.playMusic(event.newBGM);
		}
		if (event.newDialog != null) {
			displayDialog(event.newDialog);
		}
		if (event.newQuestion != null) {
			displayQuestion(event.newQuestion);
		}
	...
	public function update():void {
		...
		if (FlxG.keys.justPressed("SPACE") || FlxG.keys.justPressed("ENTER") || FlxG.mouse.justPressed()) {
			...
			if (currentDialog && currentDialog.isActive && !currentDialog.isCompleted) {
				return;
			}
