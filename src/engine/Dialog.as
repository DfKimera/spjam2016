...
public class Dialog extends FlxGroup {
	...
	public function Dialog(scene:Scene, character:Character, message:String, expression:String = "default", position:String = "bottom") {
		...
		title = new FlxText(titleOffset[0], Number(titleOffset[1]) + offsetY, titleOffset[2], character.characterName);
		title.setFormat("merriweather", 18, character.textColor, "left", 0xFF000000);
		add(title);

		text = new FlxText(textOffset[0], Number(textOffset[1]) + offsetY, textOffset[2], "");
	...
	private function resetPortraitPosition():void {
		...
		portrait.x = FlxG.stage.width - Number(portrait.width) - Number(portraitOffset[0]);
		portrait.y = Number(portraitOffset[1]) + offsetY;
		portrait.y = Number(portrait.y) - Number(portrait.height);
	}
