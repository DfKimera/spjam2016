 ...
 */
 class CenterSlideFX extends BaseFX {
	 ...
	 */
	public var completeCallback:ASFunction;
	...
	public static inline final REVEAL_VERTICAL:UInt = 0;
	public static inline final REVEAL_HORIZONTAL:UInt = 1;
	public static inline final HIDE_VERTICAL:UInt = 2;
	public static inline final HIDE_HORIZONTAL:UInt = 3;
	...
	public function createFromFlxSprite(source:FlxSprite, direction:UInt = 0, pixels:UInt = 1, backgroundColor:UInt = 0x0):FlxSprite {
		return create(source.pixels, Std.int(source.x), Std.int(source.y), direction, pixels, backgroundColor);
	}
	 ...
	 * @return	An FlxSprite with the effect running through it, which should be started with a call to CenterSlideFX.start()
	 */
	public function createFromClass(source:Class<Dynamic>, x:Int, y:Int, direction:UInt = 0, pixels:UInt = 1, backgroundColor:UInt = 0x0):FlxSprite {
	...
	function create(source:BitmapData, x:Int, y:Int, direction:UInt = 0, pixels:UInt = 1, backgroundColor:UInt = 0x0):FlxSprite {
		...
		var midway= Std.int(source.height / 2);
	...
	public function draw() {
		if (ready && complete == false) {
			...
			if (complete && Reflect.isFunction(completeCallback )) {
				completeCallback();
			}
		}
	}
