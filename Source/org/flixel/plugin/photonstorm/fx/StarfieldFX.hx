 ...
 */
 class StarfieldFX extends BaseFX {
	...
	var stars:Array<ASAny>;
	var starfieldType:Int = 0;

	var backgroundColor:UInt = 0xff000000;

	var updateSpeed:Int = 0;
	var tick:Int = 0;

	var depthColours:Array<ASAny>;

	public static inline final STARFIELD_TYPE_2D= 1;
	public static inline final STARFIELD_TYPE_3D= 2;
	 ...
	 */
	public function setBackgroundColor(backgroundColor:UInt) {
		clsColor = backgroundColor;
	}
	 ...
	 */
	public function setStarDepthColors(depth:Int, lowestColor:UInt = 0xff585858, highestColor:UInt = 0xffF4F4F4) {
	 ...
	 */
	public function setStarSpeed(xShift:Float, yShift:Float) {
		starXOffset = xShift;
		starYOffset = yShift;
	}
	 ...
	 */
	@:flash.property public var speed(get,set):Int;
function  get_speed():Int {
		return updateSpeed;
	}
function  set_speed(newSpeed:Int):Int{
		return updateSpeed = newSpeed;
	}

	function update2DStarfield() {
		for (star in stars) {
			star.x = ASCompat.toNumber(star.x) + (starXOffset * ASCompat.toNumber(star.speed));
			star.y = ASCompat.toNumber(star.y) + (starYOffset * ASCompat.toNumber(star.speed));

			canvas.setPixel32(Std.int(ASCompat.toNumber(star.x)), Std.int(ASCompat.toNumber(star.y)), depthColours[Std.int(ASCompat.toNumber(star.speed) - 1)]);
	...
	function update3DStarfield() {
		for (star in stars) {
			star.d = ASCompat.toNumber(star.d) * 1.1;
			star.x = centerX + (Math.cos(ASCompat.toNumber(star.r)) * ASCompat.toNumber(star.d)) * ASCompat.toNumber(star.speed);
			star.y = centerY + (Math.sin(ASCompat.toNumber(star.r)) * ASCompat.toNumber(star.d)) * ASCompat.toNumber(star.speed);

			star.alpha = ASCompat.toNumber(star.d) * 2;
	...
	public function draw() {
		if (Utils.getTimerStamp() > tick) {
			...
			if (updateSpeed > 0) {
				tick = Utils.getTimerStamp() + updateSpeed;
			}
		}
	}
