 ...
 */
 class FlxScrollingText extends FlxBasic {
	...
	public static function add(bitmapFont:FlxBitmapFont, region:Rectangle, pixels:UInt = 1, steps:UInt = 0, text:String = "FLIXEL ROCKS!", onlyScrollOnscreen:Bool = true, loopOnWrap:Bool = true):FlxSprite {
		...
		data.sprite = new FlxSprite(region.x, region.y).makeGraphic(Std.int(region.width), Std.int(region.height), 0x0, true);
	 ...
	 */
	public static function addText(source:FlxSprite, text:String, overwrite:Bool = false) {
		if (overwrite) {
			members[source].text = text;
		} else {
			members[source].text = ASCompat.toString(members[source].text) + text;
		}
	...
	static function scroll(data:ASAny) {
		...
		if (data.complete == false) {
			...
			if (data.x >= data.charWidth) {
				...
				if (data.complete == false) {
					data.bitmapChar = cast(data.bitmapFont, FlxBitmapFont).getCharacterAsBitmapData(ASCompat.toString(data.text).charAt(data.currentChar));
					data.x = 0;
				}
			}
