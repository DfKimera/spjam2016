 ...
 */
 class FlxGradient {
	...
	public static function createGradientMatrix(width:Int, height:Int, colors:Array<ASAny>, chunkSize:Int = 1, rotation:Int = 90):ASAny {
		var gradientMatrix= new Matrix();

		//	Rotation (in radians) that the gradient is rotated
		var rot= FlxMath.asRadians(rotation);
		...
		var alpha:Array<ASAny> = [];

		var ai= 0;while (ai < colors.length) {
			alpha.push(FlxColor.getAlphaFloat(colors[ai]));
ai++;
		}

		var ratio:Array<ASAny> = [];
		...
		if (colors.length == 2) {
			...
			//	Spread value
			var spread= Std.int(255 / (colors.length - 1));

			ratio.push(0);

			var ri= 1;while (ri < colors.length - 1) {
...
ri++;
	...
	public static function createGradientArray(width:Int, height:Int, colors:Array<ASAny>, chunkSize:Int = 1, rotation:Int = 90, interpolate:Bool = true):Array<ASAny> {
		var data= createGradientBitmapData(width, height, colors, chunkSize, rotation, interpolate);

		var result:Array<ASAny> = [];

		var y= 0;while (y < data.height) {
...
y++;
	 ...
	 */
	public static function createGradientFlxSprite(width:Int, height:Int, colors:Array<ASAny>, chunkSize:Int = 1, rotation:Int = 90, interpolate:Bool = true):FlxSprite {
		var data= createGradientBitmapData(width, height, colors, chunkSize, rotation, interpolate);

		var dest= new FlxSprite().makeGraphic(width, height);
	...
	public static function createGradientBitmapData(width:Int, height:Int, colors:Array<ASAny>, chunkSize:Int = 1, rotation:Int = 90, interpolate:Bool = true):BitmapData {
		//	Sanity checks
		if (width < 1) {
			width = 1;
		}
		...
		var gradient:ASAny = createGradientMatrix(width, height, colors, chunkSize, rotation);
		...
		var data= new BitmapData(width, height, true, 0x0);

		if (chunkSize == 1) {
			data.draw(s);
		} else {
			var tempBitmap= new Bitmap(new BitmapData(width, Std.int(height / chunkSize), true, 0x0));
	 ...
	 */
	public static function overlayGradientOnFlxSprite(dest:FlxSprite, width:Int, height:Int, colors:Array<ASAny>, destX:Int = 0, destY:Int = 0, chunkSize:Int = 1, rotation:Int = 90, interpolate:Bool = true):FlxSprite {
		if (width > dest.width) {
		...
		var source= createGradientFlxSprite(width, height, colors, chunkSize, rotation, interpolate);
	 ...
	 */
	public static function overlayGradientOnBitmapData(dest:BitmapData, width:Int, height:Int, colors:Array<ASAny>, destX:Int = 0, destY:Int = 0, chunkSize:Int = 1, rotation:Int = 90, interpolate:Bool = true):BitmapData {
		if (width > dest.width) {
			width = dest.width;
		}
		...
		var source= createGradientBitmapData(width, height, colors, chunkSize, rotation, interpolate);
