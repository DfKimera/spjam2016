 ...
 */
 ...
 class FlxWeapon {
	...
	var bulletSpeed:UInt = 0;
	var rotateToAngle:Bool = false;

	//	When firing from a fixed position (i.e. Missile Command)
	var fireFromPosition:Bool = false;
	var fireX:Int = 0;
	var fireY:Int = 0;

	var lastFired:UInt = 0;
	var nextFire:UInt = 0;
	var fireRate:UInt = 0;

	//	When firing from a parent sprites position (i.e. Space Invaders)
	var fireFromParent:Bool = false;
	var parent:ASAny;
	var parentXVariable:String;
	var parentYVariable:String;
	var positionOffset:FlxPoint;
	var directionFromParent:Bool = false;
	var angleFromParent:Bool = false;
	...
	//	Callbacks
	public var onPreFireCallback:ASFunction;
	public var onFireCallback:ASFunction;
	public var onPostFireCallback:ASFunction;
	...
	public static inline final BULLET_UP= -90;
	public static inline final BULLET_DOWN= 90;
	public static inline final BULLET_LEFT= 180;
	public static inline final BULLET_RIGHT= 0;
	public static inline final BULLET_NORTH_EAST= -45;
	public static inline final BULLET_NORTH_WEST= -135;
	public static inline final BULLET_SOUTH_EAST= 45;
	public static inline final BULLET_SOUTH_WEST= 135;
	...
	//	TODO :)
	var currentMagazine:UInt = 0;
	//private var currentBullet:uint;
	var magazineCount:UInt = 0;
	var bulletsPerMagazine:UInt = 0;
	var magazineSwapDelay:UInt = 0;
	var magazineSwapCallback:ASFunction;
	var magazineSwapSound:FlxSound;

	static inline final FIRE:UInt = 0;
	static inline final FIRE_AT_MOUSE:UInt = 1;
	static inline final FIRE_AT_POSITION:UInt = 2;
	static inline final FIRE_AT_TARGET:UInt = 3;
	static inline final FIRE_FROM_ANGLE:UInt = 4;
	static inline final FIRE_FROM_PARENT_ANGLE:UInt = 5;
	 ...
	 */
	public function new(name:String, parentRef:ASAny = null, xVariable:String = "x", yVariable:String = "y") {
		this.name = name;
	 ...
	 */
	public function makePixelBullet(quantity:UInt, width:Int = 2, height:Int = 2, color:UInt = 0xffffffff, offsetX:Int = 0, offsetY:Int = 0) {
		group = new FlxGroup(quantity);

		for (b in 0...quantity) {
			var tempBullet= new Bullet(this, b);
	 ...
	 */
	public function makeImageBullet(quantity:UInt, image:Class<Dynamic>, offsetX:Int = 0, offsetY:Int = 0, autoRotate:Bool = false, rotations:UInt = 16, frame:Int = -1, antiAliasing:Bool = false, autoBuffer:Bool = false) {
		group = new FlxGroup(quantity);
	 ...
	 * @param	offsetY			When the bullet is fired if you need to offset it on the y axis, for example to line it up with the "nose" of a space ship, set the amount here (positive or negative)
	 */
	public function makeAnimatedBullet(quantity:UInt, imageSequence:Class<Dynamic>, frameWidth:UInt, frameHeight:UInt, frames:Array<ASAny>, frameRate:UInt, looped:Bool, offsetX:Int = 0, offsetY:Int = 0) {
		group = new FlxGroup(quantity);
	 ...
	 */
	function runFire(method:UInt, x:Int = 0, y:Int = 0, target:FlxSprite = null, angle:Int = 0):Bool {
		if (fireRate > 0 && ((Utils.getTimerStamp() : UInt) < nextFire)) {
			return false;
		}
		...
		lastFired = Utils.getTimerStamp();
		nextFire = Utils.getTimerStamp() + fireRate;

		var launchX= Std.int(positionOffset.x);
		var launchY= Std.int(positionOffset.y);
		...
		if (method == FIRE) {
			currentBullet.fire(launchX, launchY, Std.int(velocity.x), Std.int(velocity.y));
	 ...
	 */
	public function setParent(parentRef:ASAny, xVariable:String, yVariable:String, offsetX:Int = 0, offsetY:Int = 0, useDirection:Bool = false) {
		if (parentRef) {
			fireFromParent = true;
	 ...
	 * @param	offsetY		When the bullet is fired if you need to offset it on the y axis, for example to line it up with the "nose" of a space ship, set the amount here (positive or negative)
	 */
	public function setFiringPosition(x:Int, y:Int, offsetX:Int = 0, offsetY:Int = 0) {
	 ...
	 */
	public function setBulletSpeed(speed:UInt) {
		bulletSpeed = speed;
	}
	 ...
	 */
	public function setFireRate(rate:UInt) {
		fireRate = rate;
	}
	 ...
	 */
	public function setBulletBounds(bounds:FlxRect) {
		this.bounds = bounds;
	}
	 ...
	 * @param	speed		The speed it will move, in pixels per second (sq)
	 */
	public function setBulletDirection(angle:Int, speed:UInt) {
	 ...
	 */
	public function setBulletGravity(xForce:Int, yForce:Int) {
	 ...
	 */
	public function setBulletAcceleration(xAcceleration:Int, yAcceleration:Int, xSpeedMax:Int, ySpeedMax:Int) {
	 ...
	 */
	public function setBulletOffset(offsetX:Int, offsetY:Int) {
		positionOffset.x = offsetX;
		positionOffset.y = offsetY;
	}
	 ...
	 */
	public function setBulletRandomFactor(randomAngle:UInt = 0, randomSpeed:UInt = 0, randomPosition:FlxPoint = null, randomLifeSpan:UInt = 0) {
	 ...
	 */
	public function setBulletLifeSpan(lifespan:Int) {
	 ...
	 */
	public function setBulletElasticity(elasticity:Float) {
	...
	function getFreeBullet():Bullet {
		...
		for (_tmp_ in group.members) {
var bullet:Bullet  = _tmp_;
	 ...
	 */
	public function setPreFireCallback(callback:ASFunction = null, sound:FlxSound = null) {
	 ...
	 * @param	sound		An FlxSound to play
	 */
	public function setFireCallback(callback:ASFunction = null, sound:FlxSound = null) {
	 ...
	 * @param	sound		An FlxSound to play
	 */
	public function setPostFireCallback(callback:ASFunction = null, sound:FlxSound = null) {
		onPostFireCallback = callback;
		onPostFireSound = sound;
	}

	// TODO
	public function TODOcreateBulletPattern(pattern:Array<ASAny>) {
