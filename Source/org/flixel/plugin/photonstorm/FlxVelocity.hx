...
import org.flixel.*;
 ...
 class FlxVelocity {
	 ...
	 */
	public static function moveTowardsObject(source:FlxSprite, dest:FlxSprite, speed:Int = 60, maxTime:Int = 0) {
		var a= angleBetween(source, dest);

		if (maxTime > 0) {
			var d= distanceBetween(source, dest);
	 ...
	 */
	public static function accelerateTowardsObject(source:FlxSprite, dest:FlxSprite, speed:Int, xSpeedMax:UInt, ySpeedMax:UInt) {
		var a= angleBetween(source, dest);
	 ...
	 * @param	maxTime		Time given in milliseconds (1000 = 1 sec). If set the speed is adjusted so the source will arrive at destination in the given number of ms
	 */
	public static function moveTowardsMouse(source:FlxSprite, speed:Int = 60, maxTime:Int = 0) {
		var a= angleBetweenMouse(source);

		if (maxTime > 0) {
			var d= distanceToMouse(source);
	 ...
	 * @param	ySpeedMax		The maximum speed in pixels per second in which the sprite can move vertically
	 */
	public static function accelerateTowardsMouse(source:FlxSprite, speed:Int, xSpeedMax:UInt, ySpeedMax:UInt) {
		var a= angleBetweenMouse(source);
	 ...
	 * @param	maxTime		Time given in milliseconds (1000 = 1 sec). If set the speed is adjusted so the source will arrive at destination in the given number of ms
	 */
	public static function moveTowardsPoint(source:FlxSprite, target:FlxPoint, speed:Int = 60, maxTime:Int = 0) {
		var a= angleBetweenPoint(source, target);

		if (maxTime > 0) {
			var d= distanceToPoint(source, target);
	 ...
	 * @param	ySpeedMax		The maximum speed in pixels per second in which the sprite can move vertically
	 */
	public static function accelerateTowardsPoint(source:FlxSprite, target:FlxPoint, speed:Int, xSpeedMax:UInt, ySpeedMax:UInt) {
		var a= angleBetweenPoint(source, target);
	...
	public static function distanceToMouse(a:FlxSprite):Int {
		var dx= (a.x + a.origin.x) - FlxG.mouse.screenX;
		var dy= (a.y + a.origin.y) - FlxG.mouse.screenY;
	...
	public static function velocityFromAngle(angle:Int, speed:Int):FlxPoint {
		var a= FlxMath.asRadians(angle);

		var result= new FlxPoint();
	...
	public static function angleBetweenMouse(a:FlxSprite, asDegrees:Bool = false):Float {
		...
		var dx= FlxG.mouse.screenX - p.x;
		var dy= FlxG.mouse.screenY - p.y;
