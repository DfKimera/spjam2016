 ...
 */
 class FlxScreenGrab extends FlxBasic {
	...
	static var autoHideMouse:Bool = false;
	static var region:Rectangle;
	 ...
	 */
	public static function defineCaptureRegion(x:Int, y:Int, width:Int, height:Int) {
	 ...
	 */
	public static function clearCaptureRegion() {
		region = null;
	}
	 ...
	 */
	public static function defineHotKey(key:String, saveToFile:Bool = false, hideMouse:Bool = false) {
		hotkey = key;
		autoSave = saveToFile;
	 ...
	 */
	public static function clearHotKey() {
		hotkey = "";
		autoSave = false;
		autoHideMouse = false;
	}
	...
	public static function grab(captureRegion:Rectangle = null, saveToFile:Bool = false, hideMouse:Bool = false):Bitmap {
		...
		var theBitmap= new Bitmap(new BitmapData(Std.int(bounds.width), Std.int(bounds.height), true, 0x0));

		var m= new Matrix(1, 0, 0, 1, -bounds.x, -bounds.y);
	...
	static function save(filename:String = "") {
		...
		var png= PNGEncoder.encode(screenshot.bitmapData);

		var file= new FileReference();

		if (filename == "") {
			filename = "grab" + Std.string(Utils.getTimerStamp()) + ".png";
