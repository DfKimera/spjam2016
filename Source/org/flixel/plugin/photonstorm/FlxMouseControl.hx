...
import org.flixel.*;
 ...
 class FlxMouseControl extends FlxBasic {
	...
	static var clickStack:Array<ASAny> = [];
	static var clickCoords:FlxPoint;
	static var hasClickTarget:Bool = false;

	static var oldX:Int = 0;
	static var oldY:Int = 0;

	/**
	 * The speed the mouse is moving on the X axis in pixels per frame
	 */
	public static var speedX:Int = 0;
	 ...
	 */
	public static function addToStack(item:FlxExtendedSprite) {
		if (Std.is(mouseZone , FlxRect)) {
			if (FlxMath.pointInFlxRect(Std.int(FlxG.mouse.x), Std.int(FlxG.mouse.y), mouseZone) == true) {
	 ...
	 */
	function assignClickedSprite() {
		//	If there is more than one potential target then sort them
		if (clickStack.length > 1) {
			clickStack.sort(sortHandler);
		}

		var clickStackDebug:Array<ASAny> = [];

		for (_tmp_ in clickStack) {
var i:FlxExtendedSprite  = _tmp_;
	...
	function sortHandler(item1:FlxExtendedSprite, item2:FlxExtendedSprite):Int {
		if ((item1 : ASAny)[sortIndex] < (item2 : ASAny)[sortIndex]) {
			return sortOrder;
		} else if ((item1 : ASAny)[sortIndex] > (item2 : ASAny)[sortIndex]) {
			return -sortOrder;
		}
