 ...
 */
 class FlxBar extends FlxSprite {
	 ...
	 */
	public var pxPerPercent:Float = Math.NaN;

	var emptyCallback:ASFunction;
	var emptyBar:BitmapData;
	var emptyBarRect:Rectangle;
	var emptyBarPoint:Point;
	var emptyKill:Bool = false;
	var zeroOffset:Point = new Point();

	var filledCallback:ASFunction;
	var filledBar:BitmapData;
	var filledBarRect:Rectangle;
	var filledBarPoint:Point;

	var fillDirection:UInt = 0;
	var fillHorizontal:Bool = false;

	public static inline final FILL_LEFT_TO_RIGHT:UInt = 1;
	public static inline final FILL_RIGHT_TO_LEFT:UInt = 2;
	public static inline final FILL_TOP_TO_BOTTOM:UInt = 3;
	public static inline final FILL_BOTTOM_TO_TOP:UInt = 4;
	public static inline final FILL_HORIZONTAL_INSIDE_OUT:UInt = 5;
	public static inline final FILL_HORIZONTAL_OUTSIDE_IN:UInt = 6;
	public static inline final FILL_VERTICAL_INSIDE_OUT:UInt = 7;
	public static inline final FILL_VERTICAL_OUTSIDE_IN:UInt = 8;

	static inline final BAR_FILLED:UInt = 1;
	static inline final BAR_GRADIENT:UInt = 2;
	static inline final BAR_IMAGE:UInt = 3;
	 ...
	 */
	public function new(x:Int, y:Int, direction:UInt = FILL_LEFT_TO_RIGHT, width:Int = 100, height:Int = 10, parentRef:ASAny = null, variable:String = "", min:Float = 0, max:Float = 100, border:Bool = false){
		super(x, y);
	 ...
	 */
	public function trackParent(offsetX:Int, offsetY:Int) {
		fixedPosition = false;
	 ...
	 * @param	offsetY		The offset on Y in relation to the origin x/y of the parent
	 */
	public function setParent(parentRef:ASAny, variable:String, track:Bool = false, offsetX:Int = 0, offsetY:Int = 0) {
		parent = parentRef;
		parentVariable = variable;
	 ...
	 */
	public function stopTrackingParent(posX:Int, posY:Int) {
		fixedPosition = true;
	 ...
	 */
	public function setCallbacks(onEmpty:ASFunction, onFilled:ASFunction, killOnEmpty:Bool = false) {
		if (Reflect.isFunction(onEmpty )) {
			emptyCallback = onEmpty;
		}

		if (Reflect.isFunction(onFilled )) {
	...
	@:flash.property public var killOnEmpty(get,set):Bool;
function  set_killOnEmpty(value:Bool):Bool{
		return emptyKill = value;
	}
function  get_killOnEmpty():Bool {
	...
	public function setRange(min:Float, max:Float) {
		if (max <= min) {
			throw cast("FlxBar: max cannot be less than or equal to min", Error);
			return;
		}
	...
	@:flash.property public var stats(get,never):ASAny;
function  get_stats():ASAny {
		var data:ASAny = {
			min: min,
			max: max,
			range: range,
...
;
	 ...
	 */
	public function createFilledBar(empty:UInt, fill:UInt, showBorder:Bool = false, border:UInt = 0xffffffff) {
		barType = BAR_FILLED;
	 ...
	 * @param	border		The border colour in 0xAARRGGBB format
	 */
	public function createGradientBar(empty:Array<ASAny>, fill:Array<ASAny>, chunkSize:Int = 1, rotation:Int = 180, showBorder:Bool = false, border:UInt = 0xffffffff) {
	 ...
	 */
	public function createImageBar(empty:Class<Dynamic> = null, fill:Class<Dynamic> = null, emptyBackground:UInt = 0xff000000, fillBackground:UInt = 0xff00ff00) {
		barType = BAR_IMAGE;
	 ...
	 * @param	direction 			One of the FlxBar.FILL_ constants (such as FILL_LEFT_TO_RIGHT, FILL_TOP_TO_BOTTOM etc)
	 */
	public function setFillDirection(direction:UInt) {
		switch (direction) {
			case FILL_LEFT_TO_RIGHT
			   | FILL_RIGHT_TO_LEFT
			   | FILL_HORIZONTAL_INSIDE_OUT
			   | FILL_HORIZONTAL_OUTSIDE_IN:
			...
			case FILL_TOP_TO_BOTTOM
			   | FILL_BOTTOM_TO_TOP
			   | FILL_VERTICAL_INSIDE_OUT
			   | FILL_VERTICAL_OUTSIDE_IN:
				fillDirection = direction;
	...
	function updateValueFromParent() {
		updateValue(parent[parentVariable]);
	}

	function updateValue(newValue:Float) {
		if (newValue > max) {
			newValue = max;
		}
		...
		if (value == min && Reflect.isFunction(emptyCallback )) {
			emptyCallback();
		}

		if (value == max && Reflect.isFunction(filledCallback )) {
			filledCallback();
		}
	...
	override public function update() {
		if (parent) {
			...
			if (fixedPosition == false) {
				x = ASCompat.toNumber(parent.x) + positionOffset.x;
				y = ASCompat.toNumber(parent.y) + positionOffset.y;
			}
		}
	}
	 ...
	 */
	@:flash.property public var percent(get,set):Float;
function  get_percent():Float {
		if (value > max) {
			return 100;
		}

		return Math.ffloor((value / range) * 100);
	}
...
return newPct;
	 ...
	 */
	@:flash.property public var currentValue(get,set):Float;
