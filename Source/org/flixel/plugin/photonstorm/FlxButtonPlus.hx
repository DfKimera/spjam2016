 ...
 */
 class FlxButtonPlus extends FlxGroup {
	 ...
	 */
	public var pauseProof:Bool = false;
	/**
	 ...
	 */
	public var offColor:Array<ASAny> = [0xff008000, 0xff00FF00];

	/**
	 * The color gradient of the button in its hovered state
	 */
	public var onColor:Array<ASAny> = [0xff800000, 0xffff0000];
	 ...
	 */
	public function new(X:Int, Y:Int, Callback:ASFunction, Params:Array<ASAny> = null, Label:String = null, Width:Int = 100, Height:Int = 20){
		super(4);
	...
	@:flash.property public var x(get,set):Int;
function  set_x(newX:Int):Int{
		_x = newX;
...
function  get_x():Int {
	...
	@:flash.property public var y(get,set):Int;
function  set_y(newY:Int):Int{
		_y = newY;
...
function  get_y():Int {
	 ...
	 */
	public function loadGraphic(normal:FlxSprite, highlight:FlxSprite) {
	...
	function updateButton() {
		...
		if (FlxG.mouse.visible) {
			...
			while (i < l) {
				c = ASCompat.dynamicAs(buttonNormal.cameras[i++] , FlxCamera);
		...
		if (_status != prevStatus) {
			if (_status == NORMAL) {
				...
				if (Reflect.isFunction(leaveCallback )) {
				...
				if (Reflect.isFunction(enterCallback )) {
	...
	function onMouseUp(event:MouseEvent) {
		if (exists && visible && active && (_status == PRESSED) && (_onClick != null) && (pauseProof || !FlxG.paused)) {
			Reflect.callMethod(null, _onClick, onClickParams);
		}
	}
	 ...
	 * @param	colors
	 */
	public function updateInactiveButtonColors(colors:Array<ASAny>) {
		offColor = colors;
	 ...
	 * @param	colors
	 */
	public function updateActiveButtonColors(colors:Array<ASAny>) {
		onColor = colors;
	 ...
	 */
	@:flash.property public var text(never,set):String;
...
return value;
	 ...
	 */
	public function setMouseOverCallback(callback:ASFunction, params:Array<ASAny> = null) {
	 ...
	 * @param	params		An optional array of parameters to pass to the function
	 */
	public function setMouseOutCallback(callback:ASFunction, params:Array<ASAny> = null) {
	 ...
	 * @param	params		An optional array of parameters that will be passed to the Callback function
	 */
	public function setOnClickCallback(callback:ASFunction, params:Array<ASAny> = null) {
		_onClick = callback;
