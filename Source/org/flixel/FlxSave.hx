 ...
 */
 class FlxSave {
	static var SUCCESS:UInt = 0;
	static var PENDING:UInt = 1;
	static var ERROR:UInt = 2;
	/**
	 ...
	 */
	public function close(MinFileSize:UInt = 0, OnComplete:ASFunction = null):Bool {
	 ...
	 */
	public function flush(MinFileSize:UInt = 0, OnComplete:ASFunction = null):Bool {
		if (!checkBinding()) {
			return false;
		}
	 ...
	 */
	function onFlushStatus(E:NetStatusEvent) {
	 ...
	 */
	function onDone(Result:UInt):Bool {
		switch (Result) {
			case (_ == PENDING => true):
