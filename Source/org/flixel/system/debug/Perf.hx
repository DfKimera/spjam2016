 ...
 */
 class Perf extends FlxWindow {
	...
	var _flixelUpdate:Array<ASAny>;
	var _flixelUpdateMarker:UInt = 0;
	var _flixelDraw:Array<ASAny>;
	var _flixelDrawMarker:UInt = 0;
	var _flash:Array<ASAny>;
	var _flashMarker:UInt = 0;
	var _activeObject:Array<ASAny>;
	var _objectMarker:UInt = 0;
	var _visibleObject:Array<ASAny>;
	var _visibleObjectMarker:UInt = 0;
	...
	public function new(Title:String, Width:Float, Height:Float, Resizable:Bool = true, Bounds:Rectangle = null, BGColor:UInt = 0x7f7f7f7f, TopColor:UInt = 0x7f000000) {
		...
		_flixelUpdate = Utils.makeArray(32);
		_flixelUpdateMarker = 0;
		_flixelDraw = Utils.makeArray(32);
		_flixelDrawMarker = 0;
		_flash = Utils.makeArray(32);
		_flashMarker = 0;
		_activeObject = Utils.makeArray(32);
		_objectMarker = 0;
		_visibleObject = Utils.makeArray(32);
	...
	public function update() {
		var time= Utils.getTimerStamp();
		var elapsed= time - _lastTime;
		var updateEvery:UInt = 500;
		if ((elapsed : UInt) > updateEvery) {
			elapsed = updateEvery;
		}
		_lastTime = time;

		_updateTimer += elapsed;
		if ((_updateTimer : UInt) > updateEvery) {
			var i:UInt;
			var output= "";
			...
			output += ASCompat.toNumber(ASCompat.toFixed((System.totalMemory * 0.000000954), 2)) + "MB\n";
	 ...
	 */
	public function flixelUpdate(Time:Int) {
	 ...
	 */
	public function flixelDraw(Time:Int) {
	 ...
	 */
	public function flash(Time:Int) {
	 ...
	 */
	public function activeObjects(Count:Int) {
	 ...
	 * @param Count	How many objects were updated.
	 */
	public function visibleObjects(Count:Int) {
