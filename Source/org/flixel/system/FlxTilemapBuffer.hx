 ...
 */
 class FlxTilemapBuffer {
	...
	public function new(TileWidth:Float, TileHeight:Float, WidthInTiles:UInt, HeightInTiles:UInt, Camera:FlxCamera = null) {
		...
		columns = Std.int(FlxU.ceil(Camera.width / TileWidth) + 1);
		if (columns > WidthInTiles) {
			columns = WidthInTiles;
		}
		rows = Std.int(FlxU.ceil(Camera.height / TileHeight) + 1);
	 ...
	 */
	public function fill(Color:UInt = 0) {
	 ...
	 */
	@:flash.property public var pixels(get,never):BitmapData;
function  get_pixels():BitmapData {
	 ...
	 */
	public function draw(Camera:FlxCamera, FlashPoint:Point) {
